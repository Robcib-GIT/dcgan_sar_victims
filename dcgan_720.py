...
print(f"lr_gen = {lr_gen:.4f}")
print(f"lr_dis = {lr_dis:.4f}")
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(ngf//2, ngf//4, 4, 2, 1, bias=False),     # 256x256
            nn.BatchNorm2d(ngf//4),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf//4, ngf//8, 4, 2, 1, bias=False),     # 512x512
            nn.BatchNorm2d(ngf//8),
            nn.ReLU(True),

            nn.ConvTranspose2d(ngf//8, 3, 5, 1, 2, bias=False),          # 720x720
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(3, ndf, 4, 2, 1, bias=False),     # 720 → 360
            activation_fn,
        
            nn.Conv2d(ndf, ndf * 2, 4, 2, 1, bias=False),  # 360 → 180
            nn.BatchNorm2d(ndf * 2),
            activation_fn,
        
            nn.Conv2d(ndf * 2, ndf * 4, 4, 2, 1, bias=False),  # 180 → 90
            nn.BatchNorm2d(ndf * 4),
            activation_fn,
        
            nn.Conv2d(ndf * 4, ndf * 8, 4, 2, 1, bias=False),  # 90 → 45
            nn.BatchNorm2d(ndf * 8),
            activation_fn,
        
            nn.Conv2d(ndf * 8, ndf * 16, 4, 2, 1, bias=False),  # 45 → 22
            nn.BatchNorm2d(ndf * 16),
            activation_fn,
        
            nn.Conv2d(ndf * 16, ndf * 32, 4, 2, 1, bias=False),  # 22 → 11
    ...
    def forward(self, input):
        return self.main(input).view(-1)  # da como resultado (batch_size,)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 150 == 0:
            ...
            tiempo_formateado=time.strftime("%H:%M:%S",
            time.gmtime(tiempo_transcurrido))
            ...
            if os.path.exists(archivo_excel):
                ...
                doc = ezodf.newdoc(doctype="ods", filename=archivo_excel)
            ...
            if sheet is None:
                sheet = ezodf.Sheet(nombre_hoja, size=(10000, num_columnas))  # crea con muchas filas
                doc.sheets += sheet
            ...
            doc.save()
            #print(f"[Excel] Datos guardados en fila {current_row} de la hoja '{nombre_hoja}'")
